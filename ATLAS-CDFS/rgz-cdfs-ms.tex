...
\author[Alger et al.]{M.~J.~Alger$^{1, 2}$,
  J.~K.~Banfield$^{3, 1}$,
  C.~S.~Ong$^{2, 4}$,
  O.~I.~Wong$^{5, 3}$,
  L.~Rudnick$^{6}$,
...
$^{9}$Departamento de Astronom\'ia, DCNE, Universidad de Guanajuato, Apdo. Postal 144, CP 36000, Guanajuato, Gto., Mexico
}
...
\section{Data}\label{sec:data}
  ...
  classification}\label{cross-identification-as-binary-classification}
    ...
    requires choosing the host galaxy based on its probability to be a host. One approach would be to select the candidate galaxy with the highest estimated class probability, i.e. the host galaxy is the candidate galaxy $x$ that maximises $f(x)$. This fails if there are multiple host galaxies within the $2' \times
    2'$ image, as only one host galaxy can have the maximum probability. An example of such
    ...
    and then select the candidate host galaxy with the maximum weighted probability.
    The Gaussian introduces dependence on the radio component we want to cross-identify and hence allows cross-identification of multiple host galaxies in the same image. It also acts as a prior for the host galaxy location. The width of the
