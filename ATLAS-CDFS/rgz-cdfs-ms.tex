...
\section{Data}\label{data}
  ...
  classification}\label{cross-identification-as-binary-classification}
    ...
    \begin{figure}
      \centering
      \subfloat[]{\includegraphics[width=\linewidth]{images/ARG0000u0h_first_3.pdf}\label{fig:overedge-1}}\\
      \subfloat[]{\includegraphics[width=\linewidth]{images/ARG0000u0h_first_8.pdf}\label{fig:overedge-2}}
      \caption{\protect\subref{fig:overedge-1} is a $3'$-wide radio image from Radio Galaxy
        ...
        radius. \protect\subref{fig:overedge-2} is an $8'$-wide radio image from FIRST centred
        on the same location. In this image, it is clear that the source is a
        radio double. The red box indicates the boundaries of \protect\subref{fig:overedge-1}.}
    ...
    \begin{figure}
      ...
      \caption{A radio image centred on 03$^\text{h}$~26$^\text{m}$~39.12$^\text{s}$~$-$28$^\circ$~07$'$~58.80$''$.        %ARG0003sky
  ...
  \subsection{Classifiers}\label{sec:classifiers}
    ...
    \label{sec:convolutional-neural-networks}
      ...
      \todo{Cite Gravity Spy or Galaxy Zoo 2}. We employ only a simple CNN
  ...
  \subsection{Labels}\label{labels}
    ...
    \todo{Define ``expert labels'' once and then use that everywhere instead
    of mentioning Norris every time. Plots should be fine provided the caption
  ...
  \label{sec:experimental-setup}
    ...
    of the CDFS field using Radio Galaxy Zoo cross-identifications, and
    compared the trained cross-identifiers to those trained on a set of expert
    cross-identifications of the same field. We divided the CDFS field into
    four quadrants for training and testing. The quadrants were centred on
    $\alpha = 52^\text{h}48^\text{m}00^\text{s}, \delta = -28^\circ{}06'00''$
    as shown in Figure \ref{fig:quadrants}. For each trial, one quadrant was
    used to draw test examples and the other three quadrants were used for
    training examples.
    \todo{Move this: We then applied the cross-identifiers trained on CDFS to
    the ELAIS-S1 field}.
    ...
    ATLAS objects with \citet{norris06} labels. \todo{Move what we report to
...
\section{Results}\label{results}
  ...
  \caption{Balanced accuracies for each quadrant in the galaxy
    ...
    from \citet{norris06} cross-identifications. \todo{explain colours and
  ...
  \caption{Accuracies for each quadrant in the cross-identification
    ...
    classifier simply reads the correct labels, in this case the labels
    derived from \citet{norris06}. This represents the maximum attainable
    cross-identification accuracy under our assumptions. The blue horizontal
    line indicates the average accuracy of 15 random classifiers, and hence
    represents the minimum accuracy we would expect to attain. The dashed
    lines indicate the standard deviation across CDFS quadrants and
    classifiers. \todo{explain colours and mention that colours will be
    available online.}
  ...
  \begin{table*}
    ...
    \caption{Predicted host galaxy cross-identifications for each object in
      ...
      the predictor listed as in Table \ref{tab:probs}. The
      cross-identification given by \citet{norris06} is included in the
      "Norris" column. The cross-identification given by the Radio Galaxy Zoo
      consensus is included in the "RGZ" column, along with the corresponding
      consensus levels for the radio morphology and host cross-identification
      tasks. Low radio consensus indicates that the component has multiple
      nearby components (and thus is more impacted by our assumption that the
      source is isolated), and low infrared consensus indicates that the host
      galaxy is unclear in the SWIRE image (possibly due to multiple nearby
      candidate host galaxies). Full table electronic. \todo{landscape table?
      explain dropped columns.} \cheng{What is consensus?}}
    \label{tab:cids}
  \end{table*}
