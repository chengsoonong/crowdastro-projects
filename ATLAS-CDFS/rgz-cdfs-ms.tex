...
\begin{abstract}
  ...
  trained models were used to cross-identify sources in CDFS and ELAIS-S1.
  \todo{Compare the expert and RGZ performances somehow.}  % We find that the best-performing expert-trained models had a mean accuracy
...
\section{Data}\label{sec:data}
  ...
  classification}\label{cross-identification-as-binary-classification}
    ...
    given radius \todo{Clarify what this means --- we aren't actually taking
    an infrared image}. We choose a radius of 1~arcmin to match the size of
    the images used by Radio Galaxy Zoo. We make the assumption that each
    radio image represents a single, complex extended source. This is not the
    case in general and a radio image may contain many different radio sources
    with unique host galaxies. If we had access to radio morphology
    information, we could use this to isolate relevant components and hence
    more accurately cross-identify sources, but this is beyond the scope of
    this paper. We also note that this assumption will bias our results
    against radio emission that extends beyond the 1~arcmin cutout size,
    however, these are uncommon in ATLAS with 9 radio sources identified by
    \citet{norris06} as having components further than 1~arcmin apart (and of
    these, only 5 are in Radio Galaxy Zoo). The radio cross-identification
    task then amounts to locating the host galaxy within the associated radio
    and infrared images. This is formalised as an object localisation problem:
    given a radio image and an infrared image centred on a radio component,
    locate the host galaxy of the radio component.    % S031, S336, S081, S576, S136, S719, S618, S707, S287
  ...
  \subsection{Labels}\label{sec:labels}
    \begin{figure}
      ...
      \caption{CDFS field training and testing quadrants labelled 0 -- 3. The
        ...
        \delta = -28^\circ{}06'00''$. \todo{No, it's at (52.8, -28.1)} The
        quadrants were chosen such that there are similar numbers of radio
    ...
    are both plausible. We train and test our classifiers on infrared objects
  ...
  \subsection{Application to ATLAS-CDFS}
    \begin{table*}
      ...
      \begin{tabular}{llccc}
        ...
        %  \\
        RGZ & CNN & $87.7 \pm 0.9$ & $87.7 \pm 0.9$ & $87.7 \pm 0.9$\\
    ...
    \begin{table}
      ...
      %  \\
      RGZ & CNN & $37.9 \pm 8.7$ & $86.4 \pm 2.7$\\
    ...
    on the Radio Galaxy Zoo-labelled resolved set. \todo{Re-run pipeline for
    CNN(RGZ).} Despite being lower, the accuracies attained by the classifiers
    trained on Radio Galaxy Zoo labels are still comparable to those trained
    on expert labels, with most Radio Galaxy Zoo-trained models achieving
    accuracies within a few percentage points of the expert-trained models.
    ...
    %   performance on the classification task, CNN produce a cross-identification
    %   accuracy of $93.0
    % \pm 1.5$\,per cent when trained on the expert labels. Logistic regression
    % gives consistent results on both expert ($92.8 \pm 1.5$\,per cent) and Radio
    % Galaxy Zoo labels ($92.6 \pm 1.9$\,per cent). Random forests perform much
    % more poorly on the cross-identification task than on the classification
    % task, \todo{? possibly as they produce discrete predicted probabilities ?}.
    ...
    To attempt to characterise the effect of this label noise in the test set, \todo{plot RGZ, Norris masking for CDFS.}
...
\end{figure*}
  ...
  \begin{figure}
    ...
    \caption{\todo{caption this}\label{fig:gct-to-xid}}
  \end{figure}

  \begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{gct-to-xid-precision.pdf}
    \caption{\todo{caption this}\label{fig:gct-to-xid-precision}}
  \end{figure}
...
\subsection{Application to ATLAS-ELAIS}
  ...
  \begin{table*}
    \caption{Average balanced accuracies across quadrants shown in
      \autoref{fig:elais-ba} for ELAIS. Uncertainties represent standard
      deviations. The classifiers were applied to compact sources, resolved
      sources, and all sources separately. \todo{Re-run CNN(RGZ)}}
    ...
    \begin{tabular}{ccccc}
      ...
      %  \\
      RGZ & CNN & $88.5 \pm 0.3$ & $88.5 \pm 0.3$ & $88.5 \pm 0.3$ \\
...
smaller than the variation in the CDFS field.
    ...
    \autoref{fig:cdfs-accuracies-flux} plots (a) the balanced accuracies of each
    classifier and training label set against different integrated flux
    cutoffs and (b) a histogram of integrated flux for components in the
    combined training and testing set. 99\% of radio components in the
    training and testing set have an integrated flux exceeding $0.44$~mJy.
    Classifier performance behaves similarly for different classifiers as the
    cutoff is increased, with the balanced accuracy remaining reasonably
    consistent until half of the testing points are eliminated. From this
    point the mean accuracy begins to decrease for most classifiers, though
    the standard deviation across quadrants is very high. At high \todo{write this}
    ...
    \begin{figure}
      \centering
      \includegraphics[width=\columnwidth]{images/elais-accuracies-flux.pdf}
      \caption{(a) Balanced accuracies of classifiers trained on CDFS and tested
        on ELAIS-S1 with different integrated flux cutoffs for the test set. A
        SWIRE object is included in the test set if it is within $1'$ of a radio
        component with greater integrated flux than the cutoff. Different coloured
        lines indicate different classifier/training labels combinations, where LR
        is logistic regression, RF is random forests, CNN is convolutional neural
        networks, and Norris and RGZ are the expert and Radio Galaxy Zoo label
        sets respectively. Filled areas represent standard deviations across CDFS
        training quadrants. Logistic regression and convolutional neural networks
        perform comparably across all flux cutoffs, implying that they are to some
        extent flux-independent. The same is not true for random forests: while
        random forests trained on Radio Galaxy Zoo labels perform comparably to
        other classifiers across all flux cutoffs, random forests trained on
        expert labels show a considerable drop in performance below a flux cutoff
        of $\sim 0.44$~mJy (indicated by the dashed vertical line). 99\% of CDFS
        objects in the training and testing sets have an integrated flux exceeding
        this value. (b) A histogram of integrated flux for ELAIS-S1
        radio components. Many components have less integrated flux than 99\% of
        the CDFS training and testing sets (again indicated by the dashed vertical
        line).}
      \label{fig:elais-accuracies-flux}
    \end{figure}
...
We present a machine learning approach for cross-identification of radio components with their corresponding infrared host galaxy. We use data from Radio Galaxy Zoo and ATLAS.  Expert labels were available from \citet{norris06} and crowdsourced labels from Radio Galaxy Zoo for the CDFS field.
