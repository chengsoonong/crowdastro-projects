...
\author[Alger et al.]{M.~J.~Alger$^{1, 2}$ et al.%,
  % J.~K.~Banfield$^{1, 3}$,
  % C.~S.~Ong$^{2, 4}$,
  % C.~Wolf$^{1, 3}$,
  % O.~I.~Wong$^{5, 3}$,
...
% $^{6}$Departamento de Astronom\'ia, DCNE, Universidad de Guanajuato, Apdo. Postal 144, CP 36000, Guanajuato, Gto., Mexico\\
...
% $^{9}$Minnesota Institute for Astrophysics, University of Minnesota, 116 Church St. SE, Minneapolis, MN 55455
}
...
\begin{abstract}
  ...
  the host galaxies of radio sources. While this is possible by hand, manual cross-identification is intractable for wide-area radio surveys. We
...
\section{Data}\label{sec:data}
  ...
  classification}\label{cross-identification-as-binary-classification}
    ...
    We propose a two-step method for host galaxy cross-identification. Given a radio component, we want to find the
    ...
    by a classifier. We refer to this as the `cross-identification task'. This is a two-step task as we first solve the galaxy classification task without reference to any specific radio object, and then use these results to solve the cross-identification task. Our
    approach is illustrated in \autoref{fig:windows}.

    Solving the galaxy classification task amounts to modelling a function $f$
    from the set of infrared sources $\mathcal{X}$ to the probability that an infrared
    ...
    Note that $f(x)$ is independent of any particular radio object. The galaxy classification task aims to answer the general question of whether a given galaxy is the host galaxy of \emph{any} radio component. The cross-identification task, however, attempts to cross-identify a \emph{specific} radio component. It is unclear how to pass radio component-specific information into the classifiers. Instead, we assume that the probability that a given candidate host $x$ is associated with a given radio component $r$ is the probability that $x$ is a host weighted by some radio-component specific value $g(x, r)$:
    \[
      \mbox{Pr}(x \text{ is host of } r) = f(x) g(x, r).
    \]
    We then want to maximise $\mbox{Pr}(x \text{ is host of } r)$ to solve the cross-identification task. One approach would be to select the candidate galaxy with the highest estimated class probability, i.e. take $g(x, r) = 1$ and let the host galaxy be the candidate galaxy $x$ that maximises $f(x)$. This fails if there are multiple host galaxies within the $2' \times
    2'$ image. An example of such an image is shown in \autoref{fig:broken-isolation}. Instead, we take $g$ to be a Gaussian function of
    the angular separation of the candidate host from the radio component we want to cross-identify. The Gaussian essentially acts as a prior for the host galaxy location. The width of the
    Gaussian, $\sigma$, controls the influence of the Gaussian on the final
    cross-identification. When $\sigma$ is small, our approach is
    ...
    likelihood ratios may be applied (see Weston et al. in press). A full
  ...
  \subsection{Limitations of our approach}
    ...
    A related issue is that we need to choose a window size for the radio image
    representations of each infrared object. If this image is too small, radio
    ...
    \times 32$ pixels, corresponding to approximately $48'' \times 48''$ in
    ...
    Gaussian distance weighting to correctly break ties.
  ...
  \label{vector-representation-of-infrared-sources}
    ...
    The candidate host location and infrared colours encode
    almost all information we could gain from the infrared image, so we
    do not use the infrared image in this work.
    ...
    elliptical galaxies dominated by old stellar populations. The
    ...
    The stellarity index represents how likely the object is to be a star rather than a galaxy \citep{surace05swire}.
...
\section{Results}\label{sec:results}
  ...
  \subsection{Application to ATLAS-CDFS}
    ...
    \begin{figure*}
        ...
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_14_424.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_13_415.pdf}}%
        \subfloat[]{\includegraphics[trim={0cm 1cm 3.5cm 2.6cm}, clip, width=0.33\textwidth]{images/example_12_404.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_11_306.pdf}}%
        \subfloat[]{\includegraphics[trim={0cm 1cm 3.5cm 2.6cm}, clip, width=0.33\textwidth]{images/example_2_68.pdf}}

        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_9_264.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_8_239.pdf}}
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_7_207.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_6_197.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_5_113.pdf}}%
        \subfloat[]{\includegraphics[trim={0cm 1cm 3.5cm 2.6cm}, clip, width=0.33\textwidth]{images/example_4_106.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_3_86.pdf}}%
        \subfloat[]{\includegraphics[trim={0cm 1cm 3.5cm 2.6cm}, clip, width=0.33\textwidth]{images/example_10_298.pdf}}%
        % \subfloat[]{\includegraphics[width=0.25\textwidth]{images/example_1_30.pdf}}%
...
\section{Summary}
  ...
  our cross-identification performance is not as high as desired, we
  ...
  cross-identification catalogues to other areas of the sky and can be
  incorporated into existing methods.

\section{Acknowledgements}

  This publication has been made possible by the participation of more than 11~000 volunteers in the Radio Galaxy Zoo project. Their contributions are individually acknowledged at \url{http://rgzauthors.galaxyzoo.org}. Parts of this research were conducted by the Australian Research Council Centre of Excellence for All-sky Astrophysics (CAASTRO), through project number CE110001020. Partial support for LR was provided by U.S. National Science Foundation grants AST1211595 and 1714205 to the University of Minnesota. HA benefitted from grant 980/2016-2017 of Universidad de Guanajuato. Radio Galaxy Zoo makes use of data products from the Wide-field Infrared Survey Explorer and the Very Large Array. The Wide-field Infrared Survey Explorer is a joint project of the University of California, Los Angeles, and the Jet Propulsion Laboratory/California Institute of Technology, funded by the National Aeronautics and Space Administration. The National Radio Astronomy Observatory is a facility of the National Science Foundation operated under cooperative agreement by Associated Universities, Inc. This research has made use of the NASA/IPAC Extragalactic Database (NED) which is operated by the Jet Propulsion Laboratory, California Institute of Technology, under contract with the National Aeronautics and Space Administration. The figures in this work made use of Astropy, a community-developed core Python package for Astronomy \citep{astropy}. The Australia Telescope Compact Array is part of the Australia Telescope, which is funded by the Commonwealth of Australia for operation as a National Facility managed by CSIRO. We thank A.~Tran for his comments on this manuscript.
